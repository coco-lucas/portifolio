...
export default function ProjectCard({title, isFinished, imageURL, alt, description, badge, githubURL}: ProjectProps) {
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-row justify-between items-center">
            <CardTitle className="text-center text-2xl font-extrabold">{title}</CardTitle>
            {isFinished ? (
        ...
        <CardContent>
          ...
          <div className="flex flex-row justify-between items-center">
            <div className="flex flex-wrap gap-2">
              {badge.map((element) => (
                <Badge variant="secondary" className={getBadgeClassName(element)} key={element}>
                  {element}
                </Badge>
              ))}
            </div>
